...
class YandexVideoService {
  ...
  async fetchVideos(limit: number = 100): Promise<YandexVideo[]> {
    ...
    */
    ...
    // Auto-discover local videos from manifest generated at build/deploy time
    let videoFiles: string[] = []
    try {
      const bust = Date.now();
      const manifest = await axios.get<string[]>(`/videocircles/manifest.json?bust=${bust}`, { headers: { 'Cache-Control': 'no-cache' } })
      videoFiles = manifest.data || []
    } catch {
      // Fallback to minimal list if manifest missing (exclude placeholder)
      videoFiles = []
    }
